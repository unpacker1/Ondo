#!/bin/bash
# SKYWATCH v7 — Tüm 28 öneri entegre edildi
...
HTML_PATH = os.path.join(D, "sw7.html")
...
w(['<!DOCTYPE html>',
   ...
   '.dark-theme{background:#030c14;color:#bbffdd;}',  # Gece modu arayüz
   ...
   'async function initWorker(){worker=new Worker("worker.js");worker.onmessage=handleWorkerMessage;}',
   ...
   'window.addEventListener("load",()=>{let saved=localStorage.getItem("sw6tok");if(saved)document.getElementById("ti").value=saved;let dark=localStorage.getItem("darkTheme")==="true";if(dark)toggleDarkTheme();let lang=localStorage.getItem("lang")||"tr";setLanguage(lang);});',
   ...
   'function setLanguage(l){lang=l;localStorage.setItem("lang",l);document.querySelectorAll("[data-lang]").forEach(el=>el.textContent=t(el.dataset.lang));}',
   '</script></body></html>'])
WRITEPY
