...
function genDemo(){
  ...
  return Array.from({length:80}, (_,i) => [
    'dm'+i, al[i%al.length]+(100+i), co[i%co.length], null, null,
...
function initMap(){
  ...
  map.on('error',()=>showNtf('Harita hatası',true));
}
...
echo -e "  \( {C}Tarayıcıda açılıyor... \){N}\n"

# Termux'ta en iyi çalışan tarayıcılar
if command -v termux-open-url &>/dev/null; then
  termux-open-url "file://$HTML"
elif command -v xdg-open &>/dev/null; then
  xdg-open "file://$HTML"
else
  echo -e "\( {Y}Tarayıcı açılamadı. Manuel olarak şu dosyayı açın: \){N}"
  echo "   file://$HTML"
fi
