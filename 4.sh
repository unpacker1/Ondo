#!/bin/bash
# ╔══════════════════════════════════════════════════════╗
# ║  SKYWATCH — Termux All-in-One Launcher (ENHANCED)    ║
...
echo "  ╚══════╝╚═╝  ╚═╝   ╚═╝    ╚══╝╚══╝ ╚═╝  ╚═╝   ╚═╝    ╚═════╝╚═╝  ╚═╝"
echo -e "${N}"
echo -e "  \( {C}Canli Ucak Takip — OpenSky + Mapbox Uydu (Enhanced Edition) \){N}"
echo "  ──────────────────────────────────────────────────────"
echo ""
...
"var trails = {}; // icao -> array of {lon,lat}\n"
"var trailLayerId = 'trails';\n"
"var aircraftLayerId = 'aircraft';\n"
"var lastDataTime = 0;\n"
...
"    type: 'line',\n"
"    source: trailLayerId,\n"
"    paint: {'line-color':'#00ff88','line-width':1.5,'line-opacity':0.35}\n"
"  });\n"
"  // Aircraft symbol layer (performans için)\n"
...
"    },\n"
"    paint: {'icon-color': ['case',['==',['get','selected'],true],'#00e5ff','#00ff88']}\n"
"  });\n"
"  // Custom plane image (SVG base64)\n"
...
"  map.loadImage(planeSVG, function(err, image){\n"
"    if(!err) map.addImage('plane', image, {pixelRatio:2});\n"
"  });\n"
"}\n"
...
"    fl.innerHTML='<div style=\"padding:20px;text-align:center;color:rgba(168,255,212,.3);font-size:11px\">EŞLEŞEN UÇAK YOK</div>';\n"
"  }\n"
"}\n"
...
"  });\n"
"  renderList(filtered);\n"
"  // Harita için filtre uygulamıyoruz (tüm uçaklar görünsün), sadece liste\n"
"}\n"
...
"  if(map) renderMarkers(); // selected rengi için\n"
"  applyFilters();\n"
"  // Sesli uyarı (Türkiye'ye yakınsa)\n"
"  if(f.country === 'Turkey' && f.alt && f.alt < 10000){\n"
...
"    for(var i=0;i<4;i++){\n"
"      ctx.beginPath();\n"
"      ctx.arc(48,48,12+i*18,0,Math.PI*2);\n"
"      ctx.stroke();\n"
"    }\n"
"    radarA = (radarA + 8) % 360;\n"
"    ctx.save();\n"
...
"  // default dark ama kullanıcı isterse değiştirir\n"
"}\n"
...
echo -e "  \( {G}SKYWATCH Enhanced başlatılıyor... \){N}"
echo -e "  \( {C}Tarayıcıda açılıyor... \){N}"
termux-open-url "file://$HTML" || echo -e "  ${Y}Manuel olarak \( HTML dosyasını tarayıcıda açın. \){N}"

echo ""
echo -e "\( {G}✓ Tüm iyileştirmeler eklendi: \){N}"
echo "   • Türkiye odaklı bounding box"
echo "   • Uçak iz (trail) sistemi"
echo "   • Symbol Layer (yüksek performans)"
echo "   • Filtreler: Türkiye / Yüksek / Hızlı + arama"
echo "   • TR sayacı"
echo "   • Klavye kısayolları (R=yenile, ESC=kapat...)"
echo "   • Flightradar24 linki"
echo "   • Sesli uyarı (Türkiye + alçak uçak)"
echo "   • Fullscreen butonu"
echo "   • Gece modu algılama"
echo ""
echo -e "\( {C}Keyifli takipler! \){N}"