...
# Python kontrol
# Hata burada idi, düzeltildi: `if! command...; then` yerine `if! command...; then`
# veya daha güvenli bir şekilde `if! command... &&! command...; then` olarak yazılmalı.
# En doğru kullanım için `if! (command -v python3 && command -v python) &>/dev/null; then`
# veya her iki komutun da olmadığını kontrol eden daha okunaklı bir yol.
# Senin kodunda `!` ve `command` arasında bir boşluk olmalıydı ve `[[...]]` kullanmak daha iyi.
...
function createEl(hdg,sel){
  ...
  el.innerHTML='<svg viewBox=\"0 0 24 24\" fill=\"none\" style=\"transform:rotate('+(hdg||0)+'deg);width:100%;height:100%\"><path d=\"M12 2L8 10H4L6 12H10L8 20H12L16 12H20L22 10H18L12 2Z\" fill=\"'+(sel?'#00e5ff':'#